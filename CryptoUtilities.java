...
public class CryptoUtilities {
    ...
    public static final int PRIME_LEN = 512;
    
    public static final int PRIME_CERTAINTY = 3;
    
    public static BigInteger getRandomPrime() {
    	return new BigInteger(PRIME_LEN, PRIME_CERTAINTY, new Random());
    }
    
    public static BigInteger getSafePrime(BigInteger starterPrime) {
    	boolean isPrime = false;
    	BigInteger finalPrime = null;
    	Random rng = new Random();
    	
    	while (!isPrime) {
    		starterPrime = new BigInteger(PRIME_LEN, PRIME_CERTAINTY, rng);
    		finalPrime = starterPrime.multiply(new BigInteger("2"));
    		finalPrime = finalPrime.add(new BigInteger("1"));
    		if (finalPrime.isProbablePrime(PRIME_CERTAINTY))
    			isPrime = true;
    	}
    ...
    public static BigInteger getPrimitiveRoot(BigInteger primeFactor, BigInteger modulo) {
    	BigInteger moduloMinusOne = modulo.subtract( BigInteger.valueOf(1) );
    	for (	BigInteger g = BigInteger.valueOf(1); // Set g to 1
    			g.compareTo(moduloMinusOne) >= 0; // If g >= p - 1 then cancel
    			g = g.add( BigInteger.valueOf(1) ) // Increment g by one
    			) {
    		// If g^q != 1 (mod p):
    		if (g.modPow(primeFactor, modulo).compareTo( BigInteger.valueOf(1) ) != 0) {
    			// g is good!  Return g
    			return g;
    		}
    		// Otherwise, g was not good; increment and continue
    	}
